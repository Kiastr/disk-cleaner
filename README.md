...
- **默认自动识别**（扫描后自动进名单，对标 CCleaner / BleachBit 默认清理项）：
  - 🟢 **缓存目录**：浏览器/应用缓存（`Cache`、`Code Cache`、`GPUCache`、`ShaderCache`、`CacheStorage`、`Blob Storage` 等）
  - 🟢 **临时目录**：`Temp` 等
  - 🟢 **系统缓存**：Windows Update 下载/数据缓存、传递优化、Prefetch
  - 🟢 **缩略图缓存**：ThumbCache
- **需手动加入**（扫描结果里出现，点「加入名单」才进名单）：日志目录、崩溃报告、垃圾文件（.tmp/.dmp/.log 等）、更新残留、开发缓存（npm/pip 等）——避免误清
- **排序与筛选**：「大小」列表头点击排序（默认降序，最大的排最前）；「分类」列下拉筛选
- 注：规则只匹配**纯缓存/临时**类路径，**不含浏览器 Cookie 与历史记录**（登录态与隐私数据，见「工作原理」）
...
### 探索清理 · Treemap 磁盘热力图
> 实测数据：共命中 **19 138 MB**（可删 **17 448.7 MB** / 需确认 **1 689.3 MB**）· **144 555** 文件 · **25.1s** · **670** 疑似项
...
3. **收名单**：点「加入名单」把疑似项收进清理名单（缓存/临时类已自动加入）
...
| 等级 | 含义 | 处理策略 |
| --- | --- | --- |
| 🟢 `safe` | 纯缓存 / 临时文件 / 日志 / 崩溃报告，删除无副作用 | 其中缓存/临时类**自动加入清理名单**；日志、崩溃报告等需手动加入，可一键清理 |
